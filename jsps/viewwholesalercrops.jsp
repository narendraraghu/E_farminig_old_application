 ...
 <form action=""  name="budget">
   ...
   <caption></caption>
     ...
     <tr><td>District Name</td><td> 
				...
				<c:choose>
				 <c:when test="${requestScope.distname ne null}">
				   <option value="${requestScope.distname}">${requestScope.distname}</option>
				 </c:when>
				 <c:otherwise>
				...
				<select name="cname" id="select" onchange="javascript:if(document.budget.dname1.value==''){alert('Select Any Department');}  {var statename=document.getElementById('dname').value;var dist=document.getElementById('dname1').value;var crop=document.getElementById('cname').value;location.href='./ViewCropsAvailablityAction?distname='+dist+'&statename='+statename+'&cropname='+crop}">
				<c:choose>
				 <c:when test="${requestScope.crop ne null}">
				   <option value="${requestScope.crop}">${requestScope.crop}</option>
				 </c:when>
				 <c:otherwise>
				   <option value="">--SELECT--</option>
				   <c:if test="${not empty trans}">
				<c:forEach var="depart" items="${trans}">
				<option value="${depart.cropname}">								
				${depart.cropname}
				</option>
				</c:forEach>
				</c:if>
